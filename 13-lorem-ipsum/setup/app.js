...
const paragraphAmount = document.getElementById('amount');
...
form.addEventListener('submit', (e)=>{
  ...
  // empty, less than 0, greater than 
  if(isNaN(value) || value <= 0 || value > 9){
    output.innerHTML = `<p class="result">${text[randomPara]}<p>`;
  }else{
    let tempPara = text.slice(0,value);
    tempPara = tempPara.map((para)=>{
      return `<p class="result">${para}</p>`;
    }).join('');
    output.innerHTML = tempPara;
  }
});